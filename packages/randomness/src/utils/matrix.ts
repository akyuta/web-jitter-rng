...
export const computeRank = (M: number, Q: number, matrix: Matrix): number => {
  ...
  for (let i = 0; i < m - 1; i++) {
    if (matrix[i][i] === 1) perform_elementary_row_operations(MATRIX_FORWARD_ELIMINATION, i, M, Q, matrix);
  ...
  for (let i = m - 1; i > 0; i--) {
    if (matrix[i][i] === 1) perform_elementary_row_operations(MATRIX_BACKWARD_ELIMINATION, i, M, Q, matrix);
...
const perform_elementary_row_operations = (
  ...
  if (flag === MATRIX_FORWARD_ELIMINATION) {
    for (let j = i + 1; j < M; j++)
      if (A[j][i] === 1) for (k = i; k < Q; k++) A[j][k] = (A[j][k] + A[i][k]) % 2;
  } else {
    for (let j = i - 1; j >= 0; j--)
      if (A[j][i] === 1) for (k = 0; k < Q; k++) A[j][k] = (A[j][k] + A[i][k]) % 2;
  }
};

const find_unit_element_and_swap = (flag: number, i: number, M: number, Q: number, A: Matrix): number => {
  let index: number;
  if (flag === MATRIX_FORWARD_ELIMINATION) {
    index = i + 1;
