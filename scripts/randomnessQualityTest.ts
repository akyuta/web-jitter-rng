import { getJitterRandom, collectJitterBytes } from '../packages/web-jitter-rng/src/index';
// @ts-ignore
...
async function runQualityTest() {
    ...
    if (allPass) {
        ...
        if (isRaw) {
            console.log('NOTE: Failures in Monobit/Frequency tests are EXPECTED for raw jitter source (bias exists).');
        }
        process.exit(1);
    }
}
